...
data-autoload|false|true|声明jstree初始化完成后 是否自动去读数据
data-options|processInitTreeOptionsFunc|无|声明除了jbolt默认处理配置项 如果还需要自定义或者覆盖默认配置 指定一个function的name 有返回值即可 参考下方processInitTreeOptionsFunc
...
```
function processInitTreeOptionsFunc(tree,options){
//tree是这个树dom jquery对象
//options是默认配置 可以继续扩展
//举例 options["core"]["data"]={数据}
}
```
