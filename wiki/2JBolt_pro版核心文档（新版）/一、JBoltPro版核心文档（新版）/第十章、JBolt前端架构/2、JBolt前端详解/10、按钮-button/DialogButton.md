...
data-btn | “close” | 无 | 设置默认确定和关闭按钮的显示状态 close是只显示关闭按钮 no是不显示默认确定和关闭按钮 ok 只显示确定按钮 
...
data-ok-btn-text|"确定"|"确定"|设置ok按钮的显示文本
data-close-btn-text|"取消"|"关闭"|设置关闭按钮的显示文本
data-top-closebtn|"false"|true|设置右上角关闭按钮是否显示 默认true
