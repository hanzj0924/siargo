var jbolt_table_js_version="3.9.1";
 ...
 * @param table
 */
function activeCurrentFocusTdToEditor(table){
 ...
 * @param tableEle            表格对象 组件 或者关联了data-table-id的元素
 * @param tr                  指定行
 * @param attrValues          //[{attr:"age",value:1},{attr:"sex",value:1}]
 * @param dontExeValueChangeHandler      改变单元格的值 但是不触发任何changeColumns和handler事件
 */
	...
	if(isOk(table)){
		...
		if(jboltTable){
			...
			for(var i in attrValues){
				...
				jboltTable.me.setTrDataAttr(jboltTable,tr,columnJson.attr,columnJson.value,dontExeValueChangeHandler);
			}
			return true;
		}
	}
 ...
 * @param column
 */
function jboltTableHideColumn(tableEle,column){
	var table=getJBoltTableInst(tableEle);
	if(!isOk(table)){
		LayerMsgBox.alert("表格配置异常，无法找到对应表格",2);
		return false;
	}
	return table.me.hideColumn(table,column);
}
/**
 * 表格隐藏整列
 * @param tableEle
 * @param columnIndex
 */
function jboltTableHideColumnByIndex(tableEle,columnIndex){
	var table=getJBoltTableInst(tableEle);
	if(!isOk(table)){
		LayerMsgBox.alert("表格配置异常，无法找到对应表格",2);
		return false;
	}
	return table.me.hideColumnByIndex(table,columnIndex);
}
 ...
 * @param column
 */
function jboltTableShowColumn(tableEle,column){
	var table=getJBoltTableInst(tableEle);
	if(!isOk(table)){
		LayerMsgBox.alert("表格配置异常，无法找到对应表格",2);
		return false;
	}
	return table.me.showColumn(table,column);
}
/**
 * 表格显示整列
 * @param tableEle
 * @param columnIndex
 */
function jboltTableShowColumnByIndex(tableEle,columnIndex){
	var table=getJBoltTableInst(tableEle);
	if(!isOk(table)){
		LayerMsgBox.alert("表格配置异常，无法找到对应表格",2);
		return false;
	}
	return table.me.showColumnByIndex(table,columnIndex);
}
/**
	...
	if(isOk(table)){
		...
		if(jboltTable){
			...
			for(var i in columnsJsonData){
				...
				jboltTable.me.setCellEditable(jboltTable,tr,columnJson.column,columnJson.editable,columnJson.falseClear,columnJson.clearValue,dontExeValueChangeHandler);
			}
			return true;
		}
	}
		...
		if(jboltTable){
			if(confirm("确认使用此单元格数据批量赋值此列吗？")){
				...
				if(column){
					...
					if(isOk(trs)){
						var batchAssignHandlerIt = false;
						var thColumn=table.indexColumnMap["col_"+colIndex];
						if(thColumn) {
							var colConfig = table.editableOptions.cols[thColumn];
							if(colConfig.type==="select" && colConfig.handler){
								batchAssignHandlerIt = true;
							}
						}
						var selectedOptionData = useTd.data("selected-option-data");
						if(selectedOptionData && typeof(selectedOptionData)==="object" && theadTh){
							theadTh.data("batch-assign-selected-option-data",selectedOptionData);
						}
						var tr;
						trs.each(function(){
							tr=$(this);
							jboltTable.me.setCell(jboltTable,tr,column,text,value,dontExeValueChangeHandler,batchAssignHandlerIt);
						});
						theadTh.data("batch-assign-selected-option-data",null).removeAttr("data-batch-assign-selected-option-data");
						return;
					}
				}
			}
...
function resetJBolttableSlaveBox(masterTable){
	...
	var elm,elsrcurl;
	slave.find("[data-masterid],input[type='hidden'][data-masterid-hidden='true'],[data-url][data-origin-url],[href][data-origin-url],[data-url][data-src-url],[href][data-src-url]").each(function(){
		elm=$(this);
		if(elm.is("input") && elm.attr("type")==="hidden" && elm.data("masterid-hidden")){
			elm.val(id).change();
		}else{
			elsrcurl=elm.data("origin-url")||elm.data("src-url");
...
;(function($){
	...
	var jb_methods = {
		...
		checkCanSubmit:function(table){
			...
			//标识一下 即使没有change变更标识的行数据 一样全部检测必填项 用于ajax加载可编辑表格数据后没有修改直接提交情况
			if(table.editableOptions&&table.editableOptions.requiredIfNoChangedData){
				if(table.isEmpty){
					return true;
				}
				var tableChangeTrs = table.tbody.find("tr[data-changed='true']");
				if(notOk(tableChangeTrs)){
					var alltrs = table.tbody.find("tr[data-index]");
					return this.checkEditableCellRequired(table,alltrs);
				}
			}
			//执行item表格 每一个修改了的行里是否有必填的没有填写
		...
		updateOneColumnChangeSuccess:function(table,td,data){
			...
			if(beTextFormat){
				if(isOk(switchBtn)){
					originText=switchBtn.data("value");
				}else{
					originText=td.data("text");
				}
			}else{
				if(isOk(switchBtn)){
					originText=switchBtn.data("value");
				}else{
					originText=td[0].innerText;
				}
			}
//				if(beTextFormat){
//					var switchBtn=td.find("img[data-switchbtn]");
...
//						if(isOk(aftertd)){
//							aftertd.before(this.getColumnPrependElement(table,table.prependColumnType,"td"));
//						}
//					}
...
//				}
			...
			td.data("origin-text",originText).attr("data-origin-text",originText);
		...
		removeJBoltTableCheckedTr:function(table,confirm,callback){
			...
			var doRemoveFunc = function(){
				...
				if(columnprepend=="checkbox"||columnprepend.indexOf("checkbox:")!=-1||columnprepend.indexOf(":checkbox")!=-1 || columnprepend=="radio"||columnprepend.indexOf("radio:")!=-1||columnprepend.indexOf(":radio")!=-1){
					var cks=table.tbody.find("tr>td input[type='checkbox'][name='jboltTableCheckbox']:checked,tr>td input[type='radio'][name='"+table.radioGroupName+"']:checked");
					if(isOk(cks)){
		...
		getCheckedEles:function(table){
			...
			if(columnprepend=="checkbox"||columnprepend.indexOf("checkbox:")!=-1||columnprepend.indexOf(":checkbox")!=-1){
				...
				chrds=table.tbody.find("tr>td input[type='radio'][name='"+table.radioGroupName+"']:checked");
			}
			return chrds;
		},
		//获取选中了一个Tr
		...
		processCopyTrTdChanged:function(table,tr){
			...
			var tempKey,colIndex,tempTd,tdText,originText,colConfig,submitAttr,value,switchBtn;
		...
		processOneTrNewInsertTdChanged:function(table,tr,columns,data){
			...
			var tempKey,colIndex,tempTd,tdText,originText,colConfig,submitAttr,dataValue;
			for(var i in columns){
				tempKey=columns[i];
				if(tempKey){
					colIndex=table.columnIndexMap[tempKey];
		 ...
		 * @param batchAssignHandlerIt 不设置dontExeValueChangeHandler=true的时候 可用 ctrl+alt+enter批量时候用
		 *
		 */
		setCell:function(table,tr,column,text,value,dontExeValueChangeHandler,batchAssignHandlerIt){
			//设置单元格数据
			...
			if(typeof(dataIndex)!="undefined" && dataIndex>=0){
				...
				if(batchAssignHandlerIt){
					td.data("batch-assign-handler-it",true);
				}
...
//					}
		...
		hideColumnByIndex:function(table,columnIndex){
			table.table_box.find("thead>tr>th[data-col-index='"+columnIndex+"']").addClass("d-none");
			table.table_box.find("tbody>tr>td[data-col-index='"+columnIndex+"']").addClass("d-none");
			table.table_box.find("tfoot>tr>th[data-col-index='"+columnIndex+"']").addClass("d-none");
			this.afterToggleTableColumn(table);
		},
		hideColumn:function(table,column){
			var colIndex=table.columnIndexMap[column];
			this.hideColumnByIndex(table,colIndex);
		},
		showColumnByIndex:function(table,columnIndex){
			table.table_box.find("thead>tr>th[data-col-index='"+columnIndex+"']").removeClass("d-none");
			table.table_box.find("tbody>tr>td[data-col-index='"+columnIndex+"']").removeClass("d-none");
			table.table_box.find("tfoot>tr>th[data-col-index='"+columnIndex+"']").removeClass("d-none");
			this.afterToggleTableColumn(table);
		},
		showColumn:function(table,column){
			var colIndex=table.columnIndexMap[column];
			this.showColumnByIndex(table,colIndex);
		},
		afterToggleTableColumn:function(table){
...
//						if(isWithtabs()){
		...
		getColumnPrependElement:function(table,columnType,tagName,rowspan){
			...
			if(html && table.radioGroupName){
				html=html.replace("jboltTableRadio",table.radioGroupName);
			}
			return html;
		},
		...
		initTableColumnPrepend:function(table){
			...
			table.radioGroupName = table.data("column-radio-name")||("jboltTableRadio_"+randomId(5));
		},
		...
		processTableTbodyColumnPrepend:function(table){
			...
			if(isOk(tds)){
				tds.before(this.getColumnPrependElement(table,table.prependColumnType,"td"));
		...
		processTableTheadColumnPrepend:function(table,initPrepend){
			...
			if(isOk(th)){
				...
				th.before(this.getColumnPrependElement(table,table.prependColumnType,"th",table.prependRowspan));
			}
		},
		/**
		 * 处理table样式与事件主入口
		...
		processTableKeepSelectedItems:function(table){
			...
			if(!selectedItemsBoxId){
				LayerMsgBox.alert("未配置选中数据显示的区域data-selected-items-box",2);
				return;
			}
			var selectedItemsBox = jboltBody.find("#"+selectedItemsBoxId);
			if(notOk(selectedItemsBox)){
				LayerMsgBox.alert("data-selected-items-box设置的ID无效",2);
				return;
			}
			...
			if(!selectedItemsTplId){
				LayerMsgBox.alert("未配置选中数据的渲染模板data-selected-items-tpl",2);
				return;
			}
			var selectedItemsTplBox = g(selectedItemsTplId);
			if(!selectedItemsTplBox){
				LayerMsgBox.alert("选中数据的渲染模板配置有误data-selected-items-tpl",2);
				return;
			}
			...
			if(!selectedItemsTpl){
				LayerMsgBox.alert("选中数据的渲染模板配置有误 无任何有效模板数据",2);
				return;
			}
...
//				that.setTableHeight(table);
		...
		reProcessTableMain:function(table,callback){
			...
			that.processTableStyleAfterAjax(table,function(){
				if(table.editableOptions){
					...
					that.initEditableTableFirstCellAutoTrigger(table);
		...
		initEditableTableFirstCellAutoTrigger:function(table){
			if(!table.editable || !table.firstCellAutoTrigger){
				return false;
			}
			//如果按键是alt+回车键 判断执行选中
			table.isEditableLock=true;
			table.me.changeTdFocus(table);
			var td=table.tbody.focusTd;
			if(isOk(td)){
				td.trigger(table.editableOptions.trigger);
			}
...
//								this.style.display="none";
		...
		processTreeTableColumnFixed:function(table,tr,sontrs,dir){
			...
			var rn = table.radioGroupName+"_fixedheader";
			jbolt_table_fixed_trs.find("input[type='radio'][name='"+table.radioGroupName+"'],input[type='radio'][name='"+rn+"']").attr("name",""+table.radioGroupName+"_fixed");
...
//					}
		...
		getTdEditorHtml:function(table,currentTd,col_config,tdValue,tdText){
			var tpl='<div {@if tooltip} tooltip data-title="${tooltip}" {@/if} class="jbt_edit_ele ${parentCssClass}">'+
				...
				'<input {@if readOnly}readonly="readonly"{@/if} {@if focusChangeToExtraForm} data-focus-changeto-extra-form="true" {@/if} {@if jbe_id}id="${jbe_id}"{@/if} data-with-clearbtn="true" {@if ajaxCheckUrl}data-ajax-check-url="${ajaxCheckUrl}"{@/if} class="jbt_editor ${cssClass}" {@if cssStyle} style="${cssStyle}" {@/if} autocomplete="off" type="password"  data-tips="${ruleTips}" data-rule="{@if rule}${rule};{@/if}len<=${maxLength?maxLength:20}{@if min!=undefined&&min!="undefined"};>=${min}{@/if}{@if max!=undefined&&max!="undefined"};<=${max}{@/if}" data-notnull="false" maxLength="${maxLength}" placeholder ="${placeholder}" name="${jbe_col_key}" value="${tdText}"/>'+
				...
				'<input {@if readOnly}readonly="readonly"{@else}data-editable="true"{@/if} {@if focusChangeToExtraForm} data-focus-changeto-extra-form="true" {@/if} {@if ajaxCheckUrl}data-ajax-check-url="${ajaxCheckUrl}"{@/if}  class="jbt_editor ${cssClass}" {@if cssStyle} style="${cssStyle}" {@/if}  autocomplete="off" type="text" data-tips="${ruleTips}" data-rule="{@if rule}${rule};{@/if}len<=${maxLength?maxLength:20}" data-notnull="false" maxLength="${maxLength?maxLength:20}" placeholder ="${placeholder }" data-date data-type="${type}" {@if range}data-range="${range}"{@/if} data-trigger="focus" name="${jbe_col_key}" data-fmt="${pattern}" value="${tdText}"/>'+
				'{@else if type==="week" }'+
				...
				'{@else if type==="cleave_input" }'+
				'<input {@if readOnly}readonly="readonly"{@/if} {@if focusChangeToExtraForm} data-focus-changeto-extra-form="true" {@/if} {@if ajaxCheckUrl}data-ajax-check-url="${ajaxCheckUrl}"{@/if}  class="jbt_editor ${cssClass}" {@if cssStyle} style="${cssStyle}" {@/if}  autocomplete="off" type="text" data-cleave-input data-options="${options|toJsonStr}" data-tips="${ruleTips}" data-rule="{@if rule}${rule};{@/if}len<=${maxLength?maxLength:20}" data-notnull="false" maxLength="${maxLength?maxLength:20}" placeholder ="${placeholder }" name="${jbe_col_key}" value="${tdText}"/>'+
			...
			if(typeof(tdValue)=="undefined"){
				colConfig.tdValue=tdText;
			}else{
				colConfig.tdValue=tdValue;
			}
			...
			if(colConfig.changeColumns && (!colConfig.hasOwnProperty("handler")&&!colConfig.ischangecolumns)){
				...
				colConfig.handler=function(table,td,text,value,jsonData,chooseData){
					jboltTableUpdateOtherColumns(table,td.parent(),(chooseData && typeof(chooseData)==="object")?chooseData:jsonData,colConfig.changeColumns);
				}
			}
			if(col_config.type == "auto"){
		...
		processEditableEditor:function(table,currentTd,colConfig){
			...
			currentTd.data("origin-value",tdValue||"").attr("data-origin-value",tdValue||"");
			//获取到可操作和输入的控件editor
			...
			switch(configType){
				...
				case "week":
					break;
				case "cleave_input":
					JBoltCleaveInputUtil.initInput(editInput,true);
					break;
				case "autocomplete":
			...
			if(typeof(colConfig.readOnly)=="boolean" && !colConfig.readOnly){
			...
			if(configType!=="select" && typeof(colConfig.inputHandler)!=="undefined"){
				editInput.on("input",function(e){
					let dataIndex=currentTd.parent().data("index");
					let trJsonData = table.tableListDatas[dataIndex];
					colConfig.inputHandler(table,currentTd,editInput,editInput.val(),trJsonData);
					e.preventDefault();
					e.stopPropagation();
					return false;
				});
		...
		processChangeTableListDatasColumnValue:function(table,td,value){
			...
			if(typeof(value)=="string"&&colConfig&&colConfig.rule){
				if(colConfig.type=="input_number"||colConfig.type=="input_ranger"|| (typeof(colConfig.rule)=="string" && (colConfig.rule.indexOf("int")!=-1 || colConfig.rule.indexOf("number")!=-1))){
					value=Number(value);
				}
			}
			JBoltArrayUtil.changeOneItemAttrValue(table.tableListDatas,dataIndex,attrName,value);
//				console.log(table.tableListDatas)
		...
		processEditableTdChooseData:function(table,editingTd,text,value,dontExeValueChangeHandler){
			var that=this;
			var originText=editingTd.data("origin-text");
			if(typeof(originText)=="undefined"){
				originText="";
			}
			var dataOldValue = editingTd.data("value");
			if(typeof(dataOldValue) == "undefined"){
				dataOldValue="";
			}
			editingTd.data("origin-value",dataOldValue).attr("data-origin-value",dataOldValue);
...
//				var oldText = editing?"":(editingTd.data("text")||"");
			...
			var needAcHandler = false;
			var acinput = editingTd.find(".jbt_editor");
			if(isOk(acinput)){
				if(acinput.hasClass("ac_input")){
					var mustMatch = acinput.data("mustmatch");
					if(typeof(mustMatch)=="boolean" && !mustMatch){
						var exeResult = acinput.data("exe-result");
						if(typeof(exeResult)=="undefined" || !exeResult){
							needAcHandler = true;
						}
					}
				}
				//特殊处理Select
				if(acinput.is("SELECT") &&acinput.data("jbolttable-handler")){
					needAcHandler = false;
				}

			}else{
				if(''+editingTd.data("batch-assign-handler-it")==='true'){
					editingTd.isBatchAssignHandlerIt=false;
					needAcHandler = true;
					editingTd.data("batch-assign-handler-it",null).removeAttr("data-batch-assign-handler-it");
					editingTd.isBatchAssignHandlerIt=true;
				}
			}
			...
			if(typeof(text)!="undefined" && typeof(originText)!="undefined"  && text.toString()==originText.toString()){
				...
				if(needAcHandler){
					...
					if(thColumn) {
						...
						if(colConfig.handler){
							var jsonData=table.tableListDatas[editingTd.closest("tr").data("index")];
							if(!colConfig.ischangecolumns){
								this.processColConfigChangeColumns(table,editingTd,jsonData);
							}
							var chooseData = null;
							if(colConfig.type==="select"){
								if(editingTd.isBatchAssignHandlerIt){
									var theadTh=table.thead.find("tr>th[data-col-index='"+currentTdIndex+"']");
									chooseData = isOk(theadTh)?theadTh.data("batch-assign-selected-option-data"):editingTd.data("selected-option-data");
								}else{
									chooseData = editingTd.data("selected-option-data");
								}
								editingTd.isBatchAssignHandlerIt=false;
							}
							colConfig.handler(table,editingTd,text,value,jsonData,chooseData);
						}
					}
				}
			}
...
//						console.log(table.tableListDatas)
		...
		processEditable:function(table){
			...
			table.editable=true;
			table.firstCellAutoTrigger = table.data("first-cell-auto-trigger");
			//是否启用required校验的背景图效果
			var requiredImgbg=table.data("required-imgbg");
			if(typeof(requiredImgbg)=="undefined"){
				requiredImgbg=true;
...
//									that.changeTdFocus(table,td);
			...
			//定义出下面用的几个变量
			var trigger = options.trigger,colConfig,currentTd,currentTdColIndex,tempColTh,tempColKey,tempSubmitAttr,editingTd,editingInput,originText,currentTr;
			//绑定td单击事件 出现指定的编辑器
			table.table_box.on("click",function(e){
				...
				originText=currentTd.data("origin-text");
				var originTextType=typeof(originText);
				if(originTextType==="undefined"&&!originText&&!currentTd[0].hasAttribute("data-origin-text")){
...
//				that.processEditableKeyEvent(table,trigger);
		...
		processEditableKeyEvent:function(table,trigger){
			...
			jboltBody.on("keydown",function(e){
				...
				}else if((e.ctrlKey||e.metaKey)&&e.shiftKey&e.which==13){
					table.isEditableLock=false;
					that.clearTdFocus(table);
				}else{
					//按下alt
					if(!e.ctrlKey&&!e.metaKey&&!e.shiftKey&&e.altKey&&e.which==13){
						table.isEditableLock=true;
						that.processEditingTds(table);
		...
		processSwitchBtnTd:function(table,editingTd,btn,result){
			...
			var that=this;
			var originText=editingTd.data("origin-text");
			if(typeof(originText)=='undefined'&&!originText&&!editingTd[0].hasAttribute("data-origin-text")){
				originText=!result;
				editingTd.attr("data-origin-text",originText).data("origin-text",originText);
			}
			editingTd.data("text",result).attr("data-text",result);
			editingTd.data("value",result).attr("data-value",result);
			if(result.toString()===originText.toString()){
				editingTd.data("changed",false).removeAttr("data-changed");
			}else{
				editingTd.data("changed",true).attr("data-changed",true);
			}
		...
		insertRowData:function(table,tr,data,dontProcessEleInit,insertToBefore,keepId){
			...
			if(html){
				...
				if(columnPrepend&&table.prependColumnType){
					...
					if(isOk(td)){
						td.before(this.getColumnPrependElement(table,table.prependColumnType,"td"));
...
//					newTr.attr("data-tempid",tempId).data("tempid",tempId);
				...
				if(newTr && newTr.is(":last-child")){
					table.table_body[0].scrollTop=table.table_body[0].scrollHeight;
				}
			...
			if(html){
				...
				if(hasCheckedele){
					if(columnPrepend.indexOf("checkbox")!=-1){
						...
						var rdo=tr.find("div.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']");
						if(isOk(rdo)){
							rdo.click();
						}
					}
...
//						}
		...
		processColumnFixed:function(table){
			if(typeof(table.isEmpty) == "undefined"){
				//判断数据为空
				this.processEmptyTableBody(table);
			}
			//处理fixedColumn left
...
//				table.indexColumnTextMap;
		...
		processCopyTableToExcelEvent:function(table){
			...
			//按下ctrl 锁定表格
			jboltBody.on("keydown",function(e){
				table.willCopy = !e.shiftKey && !e.ctrlKey && !e.metaKey && e.altKey;
				if(table.willCopy){
...
//						td.removeClass("jbt_copy_td");
					...
					if(e.target.tagName=="A"||e.target.tagName=="BUTTON"||e.target.parentNode.tagName=="A"||e.target.parentNode.tagName=="BUTTON"){
						...
						if(rowDblClickHandler){
							if(typeof(rowDblClickHandler) == "string" && rowDblClickHandler=="edit"){
								let editBtn = tempTr1.find("td [data-role='edit']:eq(0)");
								if(!isOk(editBtn)){
									editBtn = tempTr1.find("td .jbolt_table_editbtn:eq(0)");
									if(!isOk(editBtn)){
										let thisTrIndex = tempTr1.data("index");
										let thisTr = table.tbody.find("tr[data-index='"+thisTrIndex+"']");
										if(isOk(thisTr)){
											editBtn = thisTr.find("td [data-role='edit']:eq(0)");
											if(!isOk(editBtn)) {
												editBtn = thisTr.find("td .jbolt_table_editbtn:eq(0)");
											}
										}
									}
									if(!isOk(editBtn)){
			...
			table.table_box.on("click","table>tbody>tr",function(e){
				...
				if(tagName=="TD" || (tagName=="DIV" && (e.target.className=="jbolt_table_radio"||e.target.className=="jbolt_table_checkbox"))){
					...
					if(table.prependColumnType=="checkbox"){
						...
						var rdo=table.tbody.find("tr[data-index='"+trIndex+"']>td>div.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']");
						if(isOk(rdo)){
							rdo.click();
						}
					}
		...
		processFixedColumnScrollEvent:function(table){
			...
			table.table_box.on("mousewheel",".jbolt_table_fixed",function(e){
				...
				table.table_body.animate({"scrollTop":topScroll},0,function(){
...
//				if(ajax&&!hasColumnPrepend){
			...
			table.table_box.on("click","table>thead>tr>th>.jbolt_table_radio>input[type='radio']",function(e){
				...
				if(this.checked){
					...
					if(name==table.radioGroupName+"_fixed"){
						//点击fixed表头radio
						fixedHeaderRadio=table.table_box.find("table>thead>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"_fixedheader']");
						mainRadio=table.find("thead>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']");
						mainBodyRadio=table.tbody.find("tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']");
						fixedBodyRadio=table.fixedColumnTables.find("tbody>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"_fixed']");
					}else if(name==table.radioGroupName+"_fixedheader"){
						//点击主表fixed表头radio
						mainRadio=table.find("thead>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']");
						fixedRadio=table.table_box.find("table>thead>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"_fixed']");
						mainBodyRadio=table.tbody.find("tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']");
						fixedBodyRadio=table.fixedColumnTables.find("tbody>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"_fixed']");
					}else{
						//点击主表表头radio
			...
			table.table_box.on("click","table>tbody>tr>td>.jbolt_table_radio>input[type='radio']",function(e){
				...
				var fixedHeaderRadio=table.table_box.find("table>thead>tr>th[data-col-index='"+colIndex+"']>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"_fixedheader']");
				if(isOk(fixedHeaderRadio)){
		...
		getRadioCheckedCount:function(table){
			return table.tbody.find("tr>td>.jbolt_table_radio>input[type='radio'][name='"+table.radioGroupName+"']:checked").length;
		},
...
//							sameCols.find("i.sort.sort_asc").addClass("active");
		...
		processColumnFixedLeft:function(table){
			...
			var rn = table.radioGroupName+"_fixedheader";
			jbolt_table_fixed.find("input[type='radio'][name='"+table.radioGroupName+"'],input[type='radio'][name='"+rn+"']").attr("name",""+table.radioGroupName+"_fixed");
		...
		processHeaderAndFooterFixed:function(table){
			...
			if(table.hasHeader){
				...
				var rn = table.radioGroupName+"_fixedheader";
				table.fixed_header.table.find("input[type='radio']").attr("name",rn)
			}
			if(table.hasFooter&&table.tfootFixed){
