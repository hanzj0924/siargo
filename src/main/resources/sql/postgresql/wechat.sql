...
#if(enable!=null)
 and enable=#(toSqlBool(enable))
#end
#if(isAuthenticated!=null)
 and is_authenticated=#(toSqlBool(isAuthenticated))
#end
#if(type!=null)
 and type=#(type)
#end
...
#end
#if(keywords??)
#setLocal(mpinfokw=likeValue(keywords))
...
#if(keywords??)
 and name like '%#(likeValue(keywords))%'
#end
 order by id desc
#end
...
select * from #(table) where mp_id=#(mpId) and type='#(type)'
#if(keywords??)
#setLocal(mediakw=likeValue(keywords))
...
#end
#if(keywords??)
#setLocal(userkw=likeValue(keywords))
