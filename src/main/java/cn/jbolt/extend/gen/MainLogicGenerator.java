...
public class MainLogicGenerator extends JBoltMainLogicGenerator{
	...
	public void initGenConfig() {
		...
		String packageName               = "cn.jbolt.admin.siargo.apicalllog";
		//在路由配置里的controllerPath参数 也用在生成其它URL的前缀
		String controllerPath            = "/admin/siargo/apicalllog";
		//生成html存放位置 从src/main/webapp根目录下开始 /作为前缀
		String viewFolder                = "/_view/admin/siargo/apicalllog";
		//生成Index.html左上角页面标题
		String pageTitle                 = "API调用记录";
		//在页面里使用增加 修改 删除 提示信息等用到的针对此模块操作的数据名称 例如 商品管理中是【商品】 品牌管理中是【品牌】
		String dataName                  = "ApiCallLog";
		//生成模块用的model是哪个？
		Class<? extends Model<?>> modelClass = ApiCallLog.class;
		//是否需要分页查询
		...
		SystemLogTargetType projectSystemLogTargetType   = new SystemLogTargetType("ApiCallLog","API调用记录",20005);
